...
function saveDismissed(set) { localStorage.setItem(DISMISSED_KEY, JSON.stringify([...set])); }
...
export function useNotifications() {
  ...
  const toggleEnabled = useCallback(() => {
    setEnabled((prev) => {
      const next = !prev;
      localStorage.setItem(ENABLED_KEY, next);
      if (!next) {
        // apagar: limpiar todos los timers activos
  ...
  const loadAndSchedule = useCallback(async () => {
    ...
    try {
      ...
      // si una tarea estaba vencida pero fue editada con hora futura → sacarla del historial
      if (historyRef.current.length > 0) {
        const historyIds = new Set(historyRef.current.map((h) => h.id));
        tasks.forEach((t) => {
          if (!historyIds.has(t.id) || !t.deadline_time) return;
      ...
      // tareas futuras próximas 24h → upcoming + timers
      const futureTasks = tasks.filter((t) => {
        if (!t.deadline_time || t.status === "completada") return false;
        const diff = new Date(t.deadline + "T" + t.deadline_time) - current;
      ...
      futureTasks.forEach((task) => {
        ...
        if (!scheduledWarn.current.has(task.id)) {
          const delay = new Date(taskTime.getTime() - minutesBefore * 60 * 1000) - current;
          if (delay > 0) {
            scheduledWarn.current.add(task.id);
  ...
  }, [permission, enabled, minutesBefore, addToHistory, setHistorySync]);
  ...
  useEffect(() => {
    ...
    if (enabled) {
      loadAndSchedule();
      const interval = setInterval(loadAndSchedule, 60 * 1000);
      return () => {
        timersRef.current.forEach(clearTimeout);
        clearInterval(interval);
      };
    }
  }, [loadAndSchedule, enabled]);
